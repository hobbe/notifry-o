...
public class SourceEditor extends PreferenceActivity
{
	...
	{
		// Now, send the updates to the server. On success, save the changes
		// locally.
	...
	OnPreferenceChangeListener immediateServerChangeListener = new OnPreferenceChangeListener()
	{
		...
		{
			// Crude: determine what thing we changed based on the object's
			// type.
			// CAUTION: This won't work if we need to send more than a string or
			...
			new AlertDialog.Builder(thisActivity).
					...
					setPositiveButton(
							...
							new DialogInterface.OnClickListener()
							{
								...
								{
									// Fire it off to the delete source
									// function.
	...
	OnPreferenceClickListener emailClickHandler = new OnPreferenceClickListener()
	{
		...
		public boolean onPreferenceClick( Preference preference )
		{
			...
			thisActivity.startActivity(Intent.createChooser(emailIntent, getString(R.string.source_email_key)));
		...
		public void handleMessage( Message msg )
		{
			...
			else
			{
				try
				{
					...
					{
						// Load the source from the server information. We
						// assume the server is correct.
