...
public class SpeakService extends Service implements SensorEventListener, TextToSpeech.OnInitListener
{
	...
	public void onStart( final Intent intent, int startId )
	{
		...
		else if( delaySend > 0 )
		{
			...
			// Log.d("Notifry", "Delaying message for " + delaySend +
			// " seconds.");
		...
		else
		{
			...
			// Why do we do this weird queue thing for the onInit call to work
			// with?
			// That's because if we call tts.speak() before it's initialized
			// nothing
			// happens. So the first message would 'disappear' and not be
			// spoken.
	...
	private final SpeakService alternateThis = this;
