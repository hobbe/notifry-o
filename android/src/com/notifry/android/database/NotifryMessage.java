...
/**
 * This class represents the internal message containing the notification
 * information.
 */
public class NotifryMessage extends ORM<NotifryMessage>
{
	...
	public Cursor cursorList( Context context, NotifrySource source )
	{
		...
		return context.getContentResolver().query(
				this.getContentUri(),
				new String[] { NotifryDatabaseAdapter.KEY_ID, NotifryDatabaseAdapter.KEY_TITLE, NotifryDatabaseAdapter.KEY_MESSAGE, NotifryDatabaseAdapter.KEY_TIMESTAMP, NotifryDatabaseAdapter.KEY_SEEN },
				query,
				null,
	...
	/**
	 * Return the timestamp as a long, like the unix timestamp.
	 * 
	 * @return timestamp as long, unix timestamp
	 */
	public long getLongTimestamp()
	{
		...
		try
		{
			return formatter.parse(this.timestamp).getTime();
		}
		catch( ParseException e )
		{
	...
	public static NotifryMessage fromC2DM( Context context, Bundle extras ) throws UnsourceableMessage
	{
		...
		if( source == null )
		{
			...
			// Create a dummy source as a placeholder, then ask the updater
			// service to fetch the rest
			// when it can.
			...
			if( account == null )
			{
				// Ok, not even that account is known here. We've done all we
				// can - abort!
